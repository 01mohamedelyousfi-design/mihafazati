...
describe('Taxonomy Architecture Contract (الملف التراكمي)', () => {
  ...
  it('defines 23 elements across all axes', () => {
    ...
    assert.ok(elementIds.includes('didaktiki.taqwim.asalib'), 'asalib element must exist');
    assert.ok(elementIds.includes('didaktiki.taqwim.wathaiq'), 'wathaiq element must exist');
    assert.ok(elementIds.includes('didaktiki.taqwim.bitaqat'), 'bitaqat element must exist');
  });
